...
if(flag=='c'){
	printf("in corrector-push\n");
for (m=0; m<mm; m++){
	k=(int)(zp[m]/dz);
	kp=(k+1) % km;
	wz=(float)(k+1)-zp[m]/dz;
	wzp=1.-wz;
	Exp=0.;Eyp=0.;Ezp=0.;
	rho=sqrt(2.*Mi*mu[m]/(e*e*B)); // assuming B=1, mu0=1, q=1
	for(l=0;l<4;l++){  			// 4-point gyro-average
...
//do not need to gyro-average Ez, but it makes simper code
	...
	Exp=0.25*Exp; Eyp=0.25*Eyp; Ezp=0.25*Ezp;
	vfac=0.5*vzp[m]*vzn[m]+ (e*B/Mi)*mu[m];
//using predicted v_n+1 for vac
// normalization in slab code is hard to figure out in dimensional units.  I will come 
// back to this later... I will need to define Ti at least.
	kap=kapn-(1.5-vfac)*kapt;
	xp[m]=xm[m]+0.5*dt*Eyp; // B is positive and in the z-direction
	yp[m]=ym[m]+0.5*dt*(-Exp);
	zp[m]=zm[m]+0.5*dt*vzn[m];
	vzp[m]=vzm[m]+0.5*dt*e*Ezp;
	wp[m] = wm[m]+0.5*dt*(kap*Eyp + e*Exp*vzn[m]);
...
// x_n becomes x_n-1, x_n+1 becomes x_n, in prep for next advance to time level n+1
	xm[m]=xn[m];
	ym[m]=yn[m];
	zm[m]=zn[m];
	vzm[m]=vzn[m];
